...
class CaesarCipherSimple {
    ...
    func encrypt(_ plainText: String) -> String {
        var result = ""
        let sChars: [Character] = Array(plainText)
        
        for char in sChars {
            var encrypedChar = " "
            if char != " " {
                encrypedChar = encryptMap[char]!
            }
            result.append(encrypedChar)
        }
    ...
    func decrypt(_ cipherText: String) -> String {
        var result = ""
        let sChars: [Character] = Array(cipherText)
        
        for char in sChars {
            var decryptedChar = " "
            if char != " " {
                decryptedChar = decryptMap[char]!
            }
...
class CaesarCipherAdvanced {
    ...
    func encrypt(_ plainText: String, _ n: Int = 3) -> String {
        var result = ""
        let sCharsToEncrypt: [Character] = Array(plainText)
        let lookupMap = Array(ALPHABET)
        
        for char in sCharsToEncrypt {
            var encryptedChar: Character = " "
            if char != " " {
                let x: Int = lookupMap.firstIndex(of: char)!
                let key = (x + n) % 26
    ...
    func decrypt(_ cipherText: String, _ n: Int = 3) -> String {
        var result = ""
        let sCharsToDecrypt: [Character] = Array(cipherText)
        let lookupMap = Array(ALPHABET)
        
        for char in sCharsToDecrypt {
